...
jq(function() {
	jq("#grid").jqGrid({
		...
		datatype: 'json',
		mtype: 'GET',
		colNames:['C.C Obra', 'Nome Obra'],
		colModel:[
			{name:'id',index:'id', width:60, editable:true, editoptions:{readonly:true,size:10}, hidden:false},
			{name:'nomeObra',index:'nomeObra', width:500, sorttype:"string", editable:true, editoptions:{style:"width:360px"}}
		],
	    postData: {},
	    rowNum:20,
	    rowList:[20,40,60],
...
function addRow() {
    ...
    jq("#grid").jqGrid('editGridRow','new', {
	    ...
	    beforeSubmit: function(postdata, formid){
	    	if ( rtrim(ltrim($("#id").val())) == "" ) {
	    		genericMessage("Informe o código da obra (Centro de Custo).", "Erro");
	    		return false;
	    	}
	    	
	    	
	    	if ( rtrim(ltrim($("#nomeObra").val())) == "" ) {
				genericMessage("Informe o nome da obra.", "Erro");
				return false;
			}
	    },
    	afterSubmit : function(response, postdata) { 
            ...
            if (result.success == false) {
      			for (var i = 0; i < result.message.length; i++) {
           		...
           		jq("#dialog").text('A obra foi incluída com sucesso.');
	        ...
	        afterShowForm: function(form) {
	   		...
	   		afterSubmit : function(response, postdata) { 
	            ...
	            if (result.success == false) {
	            	...
	            	jq("#dialog").text('A obra foi alterada com sucesso.');
  		...
  		jq("#grid").jqGrid( 'delGridRow', row, {
            ...
            beforeShowForm: function(form) {
                ...
                jq(".delmsg").replaceWith('<span style="white-space: pre;">' +
                   'Confirma a exclusão da obra?' + '</span>');
            ...
            afterSubmit : function(response, postdata) { 
                ...
                if (result.success == false) {
                	...
                	jq("#dialog").text('A obra foi excluída com sucesso.');
